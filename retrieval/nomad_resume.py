...
def find_resume_point():
    # finde the newest file in the directory, sort by update time
    import os
    import glob
    list_of_files = glob.glob('/data/rech/dingqian/intel/nomad/*.csv')
...
page_after_value = find_resume_point()
df = pd.read_csv(f'/data/rech/dingqian/intel/nomad/{page_after_value}.csv')
print(page_after_value)
